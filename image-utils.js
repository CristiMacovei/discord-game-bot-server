...
async function buildKingdomImage(size, buildings) {
  const underUpgrade = await loadImage(
    'assets/other/under-construction.png',
    TILE_SIZE * 0.6,
    TILE_SIZE * 0.6
  );
  ...
  for (let building of buildings) {
    ...
    if (
      building.startTimestampUnixTime + building.durationMs >
      new Date().getTime()
    ) {
      buildingImages.push({
        image: underUpgrade,
  ...
  const image = await sharp('assets/maps/simple.jpg')
    ...
    .composite(
      buildingImages.map(({ image, row, col, isIcon }) => ({
        input: image,
        left: TILE_SIZE * (col - 1) + (isIcon === true ? TILE_SIZE * 0.2 : 0),
        top: TILE_SIZE * (row - 1) + (isIcon === true ? TILE_SIZE * 0.2 : 0)
      }))
    )
    .toBuffer();
